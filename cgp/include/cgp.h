...
void set_genes_not_used(Individual *individual, int num_gates, int num_inputs);

void set_nots_on_individual(Individual *individual, int num_inputs);

int get_input_position(const char *str, int num_inputs);

int get_next_free_position(Individual *individual, int num_inputs);

int parse_expression_only_ands(Individual *individual, const char *str, int num_inputs);

int parse_expression_ands_ors(Individual *individual, const char *str, int num_inputs);

void sow_population(Individual *individual, Table *table, const char *filename, int num_gates);

/**
* @brief Function that print all truth table's data
* @param table - table struct
* @return none
*/
...
* @return none
*/
void gene_destructor(Gene *gene);
...
* @return none
*/
void individual_destructor(Individual *individual);
...
* @return none
*/
int validade_worst(Individual *individual, int worst);
...
* @return none
*/
void finds_active_genes(Individual *individual, int num_inputs_table);
...
* @return none
*/
void count_num_gates_individual(Individual *individual);
...
* @return none
*/
void apply_GAM(Individual *population, int *gates, int num_inputs_table);
...
void set_genes_not_used(Individual *individual, int num_gates, int num_inputs)
{
    int genes_not_used = individual->genotype_size - num_inputs - num_gates;
    int counter = 0;


    while(1)
    {
        int temp = randomize(num_inputs, individual->genotype_size);
...
int get_next_free_position(Individual *individual, int num_inputs)
{
    for(int i = num_inputs; i < individual->genotype_size; i++)
    {
...
int parse_expression_only_ands(Individual *individual, const char *str,  int num_inputs)
{
    ...
    if(strstr(str, "*") == NULL)
        return get_input_position(str, num_inputs);
    ...
    for (temp = (char *)str;; temp = NULL)
    {
        ...
        if (flag == 0)
        {
            input0 = get_input_position(token, num_inputs);
            flag = 1;
        }
        else if(flag == 1)
        {
            input1 = get_input_position(token, num_inputs);
            pos = get_next_free_position(individual, num_inputs);
...
int parse_expression_only_ors(Individual *individual, const char *str, int num_inputs)
{
    char *token;
    char *temp;
    char *saveptr;
...
int parse_expression_ands_ors(Individual *individual, const char *str, int num_inputs)
{
    ...
    for (temp = (char *)str;; temp = NULL)
    {
        ...
        if (flag == 0)
        {
            input0 = parse_expression_only_ands(individual, token, num_inputs);
            flag = 1;
        }
        else if (flag == 1)
        {
            input1 = parse_expression_only_ands(individual, token, num_inputs);
            pos = get_next_free_position(individual, num_inputs);
...
void individual_constructor(Individual *individual, int num_outputs_table)
{
    ...
    individual->score_per_output = (int *)malloc(sizeof(int) * num_outputs_table);
...
void print_individual(Individual *individual, int num_inputs_table)
{
    ...
    for (int i = 0; i < individual->output_size; i++)
    {
        fprintf(out_file, "%d ", individual->score_per_output[i]);
    }
    fprintf(out_file, "\n");
...
int find_worst_subgraph(Individual *individual)
{
    ...
    return validade_worst(individual, worst);
}
...
void print_post_optimization_data(Individual *individual, int num_inputs_table)
{
    ...
    //count_num_gates_individual(individual);
    print_max_depth(individual, num_inputs_table);
    print_num_gates(individual);
    //fprintf(out_file, "Num gates: %d\n", individual->num_gates);
...
int find_optimized_individual(Individual *population)
{
    count_num_gates_individual(&population[0]);
    ...
    for (int i = 1; i < NPOP; i++)
    {
        ...
        {
            count_num_gates_individual(&population[i]);
            if(population[i].num_gates <= population[best_individual].num_gates)
            {
                best_individual = i;
            }
        }
