...
int opencl_one_device(bool GPU){
    ...
    ///leituras de dados
    float** dadosTreinamento = readTrainingData(&M, &N, &NUM_CTES, &NUM_OPBIN, &NUM_OPUN, &LABELS, &conjuntoOpTerm, "f1-50.txt");
    for(int i = 0; i < M; i++){
        for(int j = 0; j < N; j++){
    ...
    if(GPU) ///escolher gpu
        cmdQueue = cl::CommandQueue(contexto, devices[2], commandQueueProperties, &result);
    else  ///escolher cpu
        cmdQueue = cl::CommandQueue(contexto, devices[0], commandQueueProperties, &result);

    //std::cout<<cmdQueue.getInfo<CL_QUEUE_DEVICE>().getInfo<CL_DEVICE_TYPE>() << std::endl;
    ...
    cmdQueue.enqueueWriteBuffer(bufferSeeds, CL_FALSE, 0, NUM_INDIV*MAX_NOS*sizeof(int), seeds);
    cmdQueue.enqueueWriteBuffer(dados, CL_FALSE, 0, M*N * sizeof(float), dadosTranspostos);
    cmdQueue.enqueueWriteBuffer(bufferOpTerm, CL_TRUE, 0, (NUM_OPBIN+NUM_OPUN+NUM_CTES+N-1)*sizeof(int), conjuntoOpTerm);
    ...
    setNDRanges(&globalSize, &localSize, &compileFlags, maxLocalSize, numPoints, cmdQueue.getInfo<CL_QUEUE_DEVICE>().getInfo<CL_DEVICE_TYPE>());
    ...
    if(GPU)
        krnlAvalia = cl::Kernel(programa, "avaliaIndividuos");
    else
        krnlAvalia = cl::Kernel(programa, "avaliaIndividuosCPU");
    ...
    while(criterioDeParada(iteracoes) /*qual o criterio de parada?*/){
        ...
        int novosIndividuos = selecionaElite(popAtual, popFutura);
        cmdQueue.enqueueWriteBuffer(bufferPopA, CL_TRUE, 0, NUM_INDIV * sizeof(Arvore), popAtual);
        cmdQueue.enqueueWriteBuffer(bufferPopF, CL_TRUE, 0, NUM_INDIV * sizeof(Arvore), popFutura);
        ...
        try {
            result = cmdQueue.enqueueNDRangeKernel(krnlEvolucao, cl::NullRange, cl::NDRange((NUM_INDIV-novosIndividuos)/2), cl::NDRange(1),NULL, &e_tempo);
        } catch(cl::Error& e){
            std::cerr << getErrorString(e.err()) << std::endl;
            exit(1);
        }
...
//        cmdQueue.enqueueReadBuffer(bufferPopF, CL_TRUE, 0, NUM_INDIV * sizeof(Arvore), popFutura);
//        cmdQueue.enqueueReadBuffer(bufferPopA, CL_TRUE, 0, NUM_INDIV * sizeof(Arvore), popAtual);
...
//        cmdQueue.enqueueWriteBuffer(bufferPopF, CL_TRUE, 0, NUM_INDIV * sizeof(Arvore), popFutura);
        ...
        try {
            result = cmdQueue.enqueueNDRangeKernel(krnlAvalia, cl::NullRange, cl::NDRange(globalSize), cl::NDRange(localSize), NULL, &e_tempo);
        } catch(cl::Error& e){
            std::cerr << getErrorString(e.err()) << std::endl;
            exit(1);
        }
...
int opencl_two_devices(bool GPU_CPU){
    std::cout << std::setprecision(32) << std::fixed;
    //comparar tudo sequencial com openmp+opencl com tudo opencl
    ...
    if(GPU_CPU){
        cmdQueueEvol = cl::CommandQueue(contexto, devices[2], commandQueueProperties, &result);
        cmdQueueAval = cl::CommandQueue(contexto, devices[0], commandQueueProperties, &result);
    } else {
        cmdQueueEvol = cl::CommandQueue(contexto, devices[0], commandQueueProperties, &result);
        cmdQueueAval = cl::CommandQueue(contexto, devices[2], commandQueueProperties, &result);
    }
    ...
    while(criterioDeParada(iteracoes) /*qual o criterio de parada?*/){
        ...
        //cmdQueueAval.enqueueReadBuffer(bufferPopA, CL_TRUE, 0, NUM_INDIV * sizeof(Arvore), popAtual);
        //popFutura = (Arvore*)cmdQueueGPU.enqueueMapBuffer(bufferPop,CL_FALSE, CL_MAP_READ, 0, NUM_INDIV * sizeof(Arvore));
