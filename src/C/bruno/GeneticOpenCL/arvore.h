...
void geradorArvore(Arvore* arv, int maxTam, int* conjuntoOpTerm, int NUM_OPBIN, int NUM_OPUN, int N, int* seed, float maxDados, float minDados);///preenche uma árvore com um numero máximo de nós 'maxTam'
...
void mutacao(Arvore* arvore, int* conjuntoOpTerm, int NUM_OPBIN, int NUM_OPUN, int N, int* seed, float maxDados, float minDados); ///Sorteia uma sub-arvore aleatória e troca por outra
void crossOver(Arvore* arvore1, Arvore* arvore2, int* seed, int id); ///Troca informações entre duas arvores
