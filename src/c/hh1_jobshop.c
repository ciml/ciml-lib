...
void opCrossover(int nGeneration, int father1, int father2);
void LOXcrossover(int nGeneration, int father1, int father2);
void PMXcrossover(int nGeneneration, int father1, int father2);
void permutation(int firstCromossome, int i, int nGeneration, int father1, int father2, int j);
...
int main()
{
  ...
  //~ {
    //~ printf("individuos = %d  ", i);
    //~ for(j = 0; j < HnumCrossover; j++)
      //~ printf("%d ", Hindividuals[i].HgCrossover[j]);
	//~ printf(" ");
    //~ printf("%.2lf ", Hindividuals[i].probCrossover);
	//~ printf(" ");
    //~ for(j = 0; j < HnumMutation; j++)
      //~ printf("%d ", Hindividuals[i].HgMutation[j]);
    //~ printf(" ");
    //~ printf("%.2lf ", Hindividuals[i].probMutation);
    //~ printf(" %.0lf", Hindividuals[i].Hfitness[2]);
    //~ printf("\n");
  //~ }
    ...
    while(nGeneration < NINDIVIDUALS)
    {
      ...
      if(x < Hindividuals[g].probCrossover)
      {
        if(Hindividuals[g].HgCrossover[0] == 1)
          opCrossover(nGeneration, father1, father2);
        if(Hindividuals[g].HgCrossover[1] == 1)
          LOXcrossover(nGeneration, father1, father2);
        if(Hindividuals[g].HgCrossover[2] == 1)
          PMXcrossover(nGeneration, father1, father2);
      }
      else
      {
        for(i = 0; i < nJobs; i++)
        {
          ...
          generation[nGeneration + 1][i] = individuals[father1][i];
		}
      }
...
//   int i, x, gver;
//
//   //Seleção das técnicas de crossover
//   do{
//     //Seleciona o ponto que ocorrerá crossover
//     x = rand() % HnumCrossover;
//
//     //Realiza o Crossover da hiperheuristica
//     for(i = 0; i < HnumCrossover; i++)
//     {
//       if(i != x)
//         Hindividuals[HnumGeneration].HgCrossover[i] = Hindividuals[Hfather1].HgCrossover[i];
...
//     }
//
//     //Verifica se somente um crossover do ag interno está ativo
//     gver = 0;
//     for(i = 0; i < HnumCrossover; i++)
//     {
//       if(Hindividuals[HnumGeneration].HgCrossover[i] == 1)
//         gver++;
//     }
//   }while(gver != 1);
//
...
//     for(i = 0; i < HnumMutation; i++)
//     {
//       if(Hindividuals[HnumGeneration].HgMutation[i] == 1)
//         gver++;
//     }
//   }while(gver != 1);
//
...
void PMXcrossover(int nGeneration, int father1, int father2)
{
  ...
  for(i = 0; i <= interval; i++)
  {
    ...
    {
      generation[nGeneration][firstCromossome + i] = individuals[father1][firstCromossome + i];
      generation[nGeneration + 1][firstCromossome + i] = individuals[father2][firstCromossome + i];
    }
  }
  ...
  for(i = 0; i <= interval; i++)
  {
    ...
    {
      if(individuals[father2][firstCromossome + i] == individuals[father1][firstCromossome + j])
      {
        vaux1 = 1;
        break;
      }
    ...
    if(vaux1 == 0)
    {
      for(j = 0; j < nJobs; j++)
      {
        if(individuals[father1][firstCromossome + i] == individuals[father2][j])
        {
          if(generation[nGeneration][j] == (-1))
          {
            generation[nGeneration][j] = individuals[father2][firstCromossome + i];
          } else
          {
            permutation(firstCromossome, i, nGeneration, father1, father2, j);
            break;
          }
        }
      }
  ...
  for(i = 0; i < nJobs; i++)
  {
    ...
    {
      generation[nGeneration][i] = individuals[father2][i];
    }
  }
  ...
  for(i = 0; i <= interval; i++)
  {
    ...
    for(j = 0; j <= interval; j++)
    {
      if(individuals[father1][firstCromossome + i] == individuals[father2][firstCromossome + j])
      {
        vaux1 = 1;
        break;
      }
    ...
    if(vaux1 == 0)
    {
      for(j = 0; j < nJobs; j++)
      {
        if(individuals[father2][firstCromossome + i] == individuals[father1][j])
        {
          if(generation[nGeneration + 1][j] == (-1))
          {
            generation[nGeneration + 1][j] = individuals[father1][firstCromossome + i];
          } else
          {
            permutation(firstCromossome, i, nGeneration + 1, father2, father1, j);
            break;
          }
        }
      }
...
void LOXcrossover(int nGeneration, int father1, int father2)
{
  ...
  for(i = 0; i < nJobs; i++)
  {
    ...
    for(j = 0; j < nJobs; j++)
    {
      if(individuals[father2][i] == generation[nGeneration][j])
      {
        gver++;
      }
    }
    if(gver == 0)
    {
      ...
      generation[nGeneration][id] = individuals[father2][i];
    }
  }
  ...
  for(i = 0; i < nJobs; i++)
  {
    ...
    for(j = 0; j < nJobs; j++)
    {
      if(individuals[father1][i] == generation[nGeneration + 1][j])
      {
        gver++;
      }
    }
    if(gver == 0)
    {
      ...
      generation[nGeneration + 1][id] = individuals[father1][i];
    }
  }
} //LOXcrossover
	...
	for(i = 0; i < nJobs; i++)
  {
		...
		for (j = 0; j < id2; j++)
    {
			if (individuals[father1][i] == generation[nGeneration + 1][j])
      {
		...
		if (gver == 0)
    {
			generation[nGeneration + 1][id2] = individuals[father1][i];
			id2++;
		}
	}
...
void permutation(int firstCromossome, int i, int nGeneration, int father1, int father2, int j)
{
  ...
  for(k = 0; k < nJobs; k++)
  {
    if(individuals[father1][j] == individuals[father2][k])
    {
      if(generation[nGeneration][k] == (-1))
      {
        generation[nGeneration][k] = individuals[father2][firstCromossome + i];
      }
      else
      {
        permutation(firstCromossome, i, nGeneration, father1, father2, k);
        break;
      }
    }
  }
