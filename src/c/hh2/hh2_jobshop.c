...
void AGinterno(int g, int h)
{
  int i, qFront[2*NINDIVIDUALS] = {0}, interval[2] = {0}, countGen = 0,
                                        aux, father1 = 0, father2 = 0, countInd;
  ...
  while(countGen < NUMGENERATION)
  {
    ...
    {
      aux = 0; // auxiliar que verifica se ocorreu algum tipo de crossover
      // Seleção dos pais
      tournament(&father1, &father2);
      for(i = 0; i < nJobs; i++)
      {
        geneFather1[i] = individuals[father1].jobsOrder[i];
        geneFather2[i] = individuals[father2].jobsOrder[i];
      }

      // Cálculo da chance de realizar crossover
      x = (double)((double)rand() / (double)RAND_MAX);
      if((x < Hindividuals[g].probCrossover) &&
                      (Hindividuals[g].HgCrossover[0] == 1))
      {
        for(i = 0; i < nJobs; i++)
        {
          individuals[countInd].jobsOrder[i] = (-1);
          individuals[countInd + 1].jobsOrder[i] = (-1);
        }
        opCrossover(countInd, geneFather1, geneFather2);
        for(i = 0; i < nJobs; i++)
        {
          geneFather1[i] = individuals[countInd].jobsOrder[i];
          geneFather2[i] = individuals[countInd + 1].jobsOrder[i];
        }
        aux = 1;
      }
      // Cálculo da chance de realizar crossover
      x = (double)((double)rand() / (double)RAND_MAX);
      if((x < Hindividuals[g].probCrossover) &&
                      (Hindividuals[g].HgCrossover[1] == 1))
      {
        for(i = 0; i < nJobs; i++)
        {
          individuals[countInd].jobsOrder[i] = (-1);
        ...
        LOXcrossover(countInd, geneFather1, geneFather2);
        for(i = 0; i < nJobs; i++)
        {
          geneFather1[i] = individuals[countInd].jobsOrder[i];
          geneFather2[i] = individuals[countInd + 1].jobsOrder[i];
      ...
      x = (double)((double)rand() / (double)RAND_MAX);
      if((x < Hindividuals[g].probCrossover) &&
                  (Hindividuals[g].HgCrossover[2] == 1))
      {
        for(i = 0; i < nJobs; i++)
        {
          individuals[countInd].jobsOrder[i] = (-1);
          ...
          PMXcrossover(countInd, geneFather1, geneFather2);
        for(i = 0; i < nJobs; i++)
        {
          geneFather1[i] = individuals[countInd].jobsOrder[i];
          geneFather2[i] = individuals[countInd + 1].jobsOrder[i];
      ...
      if((x < Hindividuals[g].probMutation) &&
                    (Hindividuals[g].HgMutation[0] == 1))
      {
        shiftMutation(countInd);
      }
      x = (double)((double)rand() / (double)RAND_MAX);
      if((x < Hindividuals[g].probMutation) &&
                    (Hindividuals[g].HgMutation[1] == 1))
      {
 ...
 void Hcrossover(int HcountInd, int Hfather1, int Hfather2)
{
   ...
   {
     if(i != x)
       Hindividuals[HcountInd].HgCrossover[i] = Hindividuals[Hfather1].HgCrossover[i];
     else
      Hindividuals[HcountInd].HgCrossover[i] = Hindividuals[Hfather2].HgCrossover[i];
   }
   ...
   {
     if(i != x)
       Hindividuals[HcountInd + 1].HgCrossover[i] = Hindividuals[Hfather2].HgCrossover[i];
     else
       Hindividuals[HcountInd + 1].HgCrossover[i] = Hindividuals[Hfather1].HgCrossover[i];
   }
   ...
   {
     if(i != x)
       Hindividuals[HcountInd].HgMutation[i] = Hindividuals[Hfather1].HgMutation[i];
     else
       Hindividuals[HcountInd].HgMutation[i] = Hindividuals[Hfather2].HgMutation[i];
   }
   ...
   {
     if(i != x)
       Hindividuals[HcountInd + 1].HgMutation[i] = Hindividuals[Hfather2].HgMutation[i];
     else
       Hindividuals[HcountInd + 1].HgMutation[i] = Hindividuals[Hfather1].HgMutation[i];
   }
 } //Hcrossover
